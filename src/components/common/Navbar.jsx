...
const NavBar = ({ setProgress }) => {
  ...
  return (
    <div
      className={` flex sm:relative bg-richblack-900 w-screen relative z-50 h-14 items-center justify-center border-b-[1px] 
        border-b-richblack-700 translate-y-  transition-all duration-500`}
    >
          ...
          <ul className=" flex-row gap-x-6 text-richblack-25 gap-5 hidden md:flex">
              ...
              <li key={index}>
                {element.title === "Catalog" ? (
                  ...
                  <div className=" flex items-center group relative cursor-pointer">
                    <p>{element.title}</p>
                    <IoIosArrowDown className="ml-1 text-xl -mr-1"/>
            ...
            >
              <input
                ...
                type="text"
                placeholder="Search"
                className=" absolute left-0 border-0 focus:ring-1 ring-richblack-400 rounded-full px-2 py-1 text-[15px] w-28 text-richblack-50 focus:outline-none focus:border-transparent bg-richblack-700"
              />
              <HiSearch
                type="submit"
                id="searchicon"
                size={20}
                className=" text-richblack-100 top-0 absolute cursor-pointer left-20"
              />
            </form>
          </ul>
        </nav>
